            ...
            <div class="p-4 sm:p-8 bg-white dark:bg-gray-800 shadow sm:rounded-lg">
                <div class="flex flex-col gap-y-4">
                    ...
                    <div>
                        ...
                        <x-text-input id="name" type="text" class="mt-1 block w-full" :value="$subject->name" required readonly />
                    </div>
                </div>
                <div class="flex flex-col gap-y-2 mt-4">
                    ...
                    <div class="flex flex-col gap-y-1">
                        @forelse($subject->groups as $group)
                            <div class="p-2 rounded flex justify-between items-center bg-gray-100 dark:bg-gray-900">
                                <a href="{{ route('groups.show', ['group' => $group->id]) }}" class="text-indigo-400 hover:text-indigo-500 transition">{{ $group->name }}</a>
                                @role('admin')
                    ...
                    @role('admin')
                        <form class="flex gap-x-2" method="POST" action="{{ route('subjects.group.add') }}">
                            ...
                            <input type="hidden" name="subject_id" value="{{ $subject->id }}">
                            <div class="w-full">
                                <x-select-input :options="$addable_groups->pluck('name', 'id')" id="group_id" name="group_id" class="block w-full" required first_empty></x-select-input>
